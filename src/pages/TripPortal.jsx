...
import {
  ...
  Bed, Car, Utensils, Sailboat, Sparkles, Send, Clock, Plane
} from 'lucide-react';
...
export default function TripPortal() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (sent) {
          setItems(await base44.entities.ProposalItem.filter({ proposal_id: sent.id }, 'day_number', 500));
        }
  ...
  const showActions = latestProposal && latestProposal.status === 'sent';
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          <div className="bg-primary text-primary-foreground px-6 py-7">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm opacity-90">
              ...
              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_STYLE[project.status] || 'bg-white/10'}`}>
                {project.status.replace(/_/g, ' ')}
              </span>
            </div>
          </div>
          <div className="p-6">
            <TripProgressTracker status={project.status} />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="bg-card rounded-2xl border border-border shadow-sm p-5">
            <h2 className="font-heading text-sm font-semibold flex items-center gap-2 mb-3 text-muted-foreground uppercase tracking-wide">
              <Calendar size={16} className="text-primary" /> Travel Dates
            </h2>
            <p className="font-heading text-lg font-semibold">
              {fmtDate(project.arrival_date)} <span className="text-muted-foreground mx-1">→</span> {fmtDate(project.departure_date)}
            </p>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-5">
            <h2 className="font-heading text-sm font-semibold flex items-center gap-2 mb-3 text-muted-foreground uppercase tracking-wide">
              <Compass size={16} className="text-primary" /> Traveller Profile
            </h2>
            <p className="text-sm leading-relaxed">
              {project.traveller_profile || `${project.travel_style || 'balanced'} style trip`}
            </p>
        ...
        {latestProposal ? (
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
            ...
            {(accommodation.length > 0 || activities.length > 0 || transport.length > 0) && (
              <div className="grid sm:grid-cols-3 gap-3 mb-6">
                {[
                  { label: 'Accommodation', icon: Bed, list: accommodation },
                  { label: 'Activities', icon: Compass, list: activities },
                  { label: 'Transport', icon: Car, list: transport },
                  ...
                  <div key={col.label} className="bg-muted/40 rounded-xl p-4">
                    ...
                    <ul className="space-y-1.5">
                      {col.list.map(i => <li key={i.id} className="text-xs leading-snug">{i.title}</li>)}
                    </ul>
                  </div>
                ))}
              </div>
            ...
            {(latestProposal.customer_notes || project.special_requests) && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                <h3 className="font-heading text-sm font-semibold text-amber-800 mb-1.5 flex items-center gap-1.5">
                  <Sparkles size={14} /> Important Notes
                </h3>
            ...
            {showActions ? (
              <div className="text-center bg-muted/30 rounded-xl p-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <Button onClick={() => act('accept')} disabled={acting} className="bg-green-600 hover:bg-green-700 text-white rounded-full px-7">
                    <Check size={18} className="mr-1" /> Accept Proposal
                  </Button>
                  <Button onClick={() => setShowChangesForm(true)} disabled={acting} variant="outline" className="rounded-full px-7">
                    <MessageSquare size={18} className="mr-1" /> Request Changes
                  </Button>
                  <Button onClick={() => { const r = prompt('Optional — tell us why:'); if (r !== null) act('decline', { decline_reason: r }); }} disabled={acting} variant="outline" className="rounded-full px-7 text-destructive border-destructive/40 hover:bg-destructive/10">
                    <XCircle size={18} className="mr-1" /> Decline
                  </Button>
                </div>
              </div>
            ) : latestProposal.status !== 'sent' && (
              <div className={`text-center rounded-xl p-6 ${latestProposal.status === 'accepted' ? 'bg-green-50 border border-green-200' : latestProposal.status === 'declined' ? 'bg-slate-50 border border-slate-200' : 'bg-amber-50 border border-amber-200'}`}>
                {latestProposal.status === 'accepted' && <Check size={26} className="mx-auto text-green-600 mb-2" />}
                {latestProposal.status === 'declined' && <XCircle size={26} className="mx-auto text-slate-500 mb-2" />}
                {latestProposal.status === 'changes_requested' && <MessageSquare size={26} className="mx-auto text-amber-600 mb-2" />}
                <h3 className="font-heading text-lg font-bold mb-0.5">
                  {latestProposal.status === 'accepted' && 'Proposal accepted!'}
                  {latestProposal.status === 'declined' && 'Proposal declined'}
                  {latestProposal.status === 'changes_requested' && 'Changes requested — thank you'}
                </h3>
                <p className="text-muted-foreground text-sm">
                  {latestProposal.status === 'accepted' && "We'll be in touch shortly to confirm the details and next steps."}
                  {latestProposal.status === 'declined' && 'Thank you for letting us know.'}
                  {latestProposal.status === 'changes_requested' && "We'll prepare an updated proposal for you soon."}
                </p>
                {latestProposal.customer_response_notes && latestProposal.status !== 'accepted' && (
                  <p className="text-xs text-muted-foreground mt-3 bg-white/60 rounded-lg p-3 text-left whitespace-pre-wrap">Your notes: {latestProposal.customer_response_notes}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-10 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-md mx-auto">
              Your trip details are with our team. We'll have your tailored itinerary ready soon — check back here or watch your messages.
            </p>
          </div>
        )}

        {/* Need to update something? / messages */}
        <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
                ...
                <div key={m.id} className="bg-muted/40 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      {m.message_type === 'change_request' ? 'Change Request' : 'Message'}
                    </span>
                  ...
                  {m.subject && <p className="font-medium text-sm mb-0.5">{m.subject}</p>}
                  ...
                  {m.admin_reply && (
                    <div className="mt-2 pt-2 border-t border-border text-sm">
                      <p className="text-xs font-semibold text-primary mb-0.5">Our reply:</p>
                      <p className="text-muted-foreground whitespace-pre-wrap">{m.admin_reply}</p>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                {[
                  ...
                  ['change_request', 'Change Request', Compass],
                ].map(([v, label, Icon]) => (
                  <button
                    key={v}
                    onClick={() => setMsg(p => ({ ...p, message_type: v }))}
                    className={`flex-1 py-2.5 rounded-xl border-2 text-sm font-medium flex items-center justify-center gap-1.5 transition-all ${
                      msg.message_type === v ? 'border-primary bg-primary/5 text-primary' : 'border-border hover:border-primary/40'
                    }`}
                  >
                    <Icon size={15} /> {label}
                  </button>
              ...
              <Button onClick={submitMessage} disabled={sendingMsg || !msg.message.trim()} className="w-full bg-primary text-primary-foreground rounded-full py-3">
                <Send size={16} className="mr-1.5" /> {sendingMsg ? 'Sending...' : 'Send to your travel team'}
              </Button>
            </div>
          )}
        </div>
        ...
        <p className="text-center text-xs text-muted-foreground pt-2">
          This is your private trip page — keep this link safe. Questions? <a href="https://wa.me/447758162004" target="_blank" rel="noopener noreferrer" className="text-primary underline">Message us on WhatsApp</a>.
        </p>
      </div>
    </div>
  );
